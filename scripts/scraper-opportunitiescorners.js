/**
 * Scraper: OpportunitiesCorners.com
 * Target: https://opportunitiescorners.com/
 * Categories: Scholarships, Fellowships, Internships, Grants, Conferences
 */
...
const BASE_URL = 'https://opportunitiescorners.com'
...
const CATEGORIES = [
  ...
  { url: `${BASE_URL}/category/exchange/`,      type: 'Exchange Programs' },
]
...
function collectPostUrls($) {
  ...
  $('article h2 a, article h3 a, h2.entry-title a, h3.entry-title a, .post-title a').each((_, el) => {
    const href = $(el).attr('href') || ''
    if (href && href.includes('opportunitiescorners.com') && !seen.has(href)) {
      seen.add(href)
      urls.push(href)
    }
  })
...
async function main() {
  console.log('=== OpportunitiesCorners Scraper ===')
  for (const cat of CATEGORIES) {
    await scrapeCategory(cat.url, cat.type)
  }
  console.log('\n=== OpportunitiesCorners scraper complete ===')
}
